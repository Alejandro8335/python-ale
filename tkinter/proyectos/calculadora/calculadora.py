...
def calcular(entry,label):
    try:
        ...
        if resultado % 1 == 0:
            ...
            entry.insert(0, float(resultado))  # Mostrar como float si hay parte decimal
...
def crear_ventana():
    ...
    for idx , num in enumerate([7,8,9,4,5,6,1,2,3]):
        button = tk.Button(ventana, text=str(num),width=4,height=2,command=lambda n=num,e=entry:agregar_numero(n,e))
        button.grid(row= 3 + idx // 3, column= idx % 3 , sticky="nsew")
        set_bts[str(num)] = button
    # botones de operaciones menos el -
    for row,column,signo in [(2,2,"%"),(2,3,"/"),(3,3,"*"),(5,3,"+")]:
        button = tk.Button(ventana, text=signo,width=4,height=2,command=lambda signo=signo,e=entry: operaciones(signo,e))# no lo toque larga historia
        button.grid(row=row, column=column,sticky="nsew")
        set_bts[signo] = button
    # linea 2(btns: clear, parentesis)
    # linea 6(btns: 0, ., ⌫, =)
    for row,column,signo,func in [(2,0,"AC",lambda e=entry:clear(e)),(2,1,"(  )",lambda e=entry:agregar_parentesis(e)),(6,2,"⌫",lambda e=entry:borrar(e)),(6,3,"=",lambda e=entry, a=label:calcular(e,a)),(6,1,".",lambda e=entry:agregar_punto(e)),(6,0,"0",lambda e=entry:agregar_numero(0,e)),(4,3,"-",lambda e=entry: sustraccion(e))]:
        button = tk.Button(ventana, text=signo,width=4,height=2,command=func)
        button.grid(row=row, column=column,sticky="nsew")
        set_bts[signo] = button
    ...
    return ventana , set_bts , entry
    
if __name__ == "__main__":
    ventana , _ , _= crear_ventana()
    ventana.mainloop()